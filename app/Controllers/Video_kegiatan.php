...
class Video_kegiatan extends BaseController
{
    ...
    public function index()
    {
        ...
        //Content
        $data['konten'] = $this->videoKegiatanModel
            ->orderby('video_kegiatan_id', 'desc')
            ->paginate(9, 'hal');

        $data['pager'] = $this->videoKegiatanModel->pager;
        ...
        return view($this->halaman . 'video_kegiatan', $data);
    }
    ...
    public function detail($slugKonten)
    {
        ...
        $data['konten'] = $this->videoKegiatanModel
            ->where('video_kegiatan_slug', $slugKonten)
            ->first();
        $data['lainya'] = $this->videoKegiatanModel
            ->orderby('rand()')
            ->limit(6)->findAll();

        return view($this->halaman . 'video_kegiatandetail', $data);
    }
}
